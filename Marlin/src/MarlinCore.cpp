...
#define CHECK_CUSTOM_USER_BUTTON(N)                                                                                 \
  do                                                                                                                \
  {                                                                                                                 \
...
void Home_Key(void)
{
  ...
  {
    if (thermalManager.degHotend(active_extruder) > 180 || thermalManager.degBed() > 50)
    {
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep /*=false*/))
{
  ...
  if (++idle_depth > 5)
    SERIAL_ECHOLNPAIR("idle() call depth: ", idle_depth);
#endif
...
// Auto-report Temperatures / SD Status
  ...
  if (!IS_SD_PRINTING() && !IS_SD_PAUSED()) // in stampa o pausa, disattiva tasti e funzioni lampeggio
  {
    ...
    {
      if ((thermalManager.degBed() <= thermalManager.degTargetBed() + 2) || (thermalManager.degHotend(active_extruder) <= thermalManager.degTargetHotend(active_extruder) + 2))
      {
        if (thermalManager.degBed() <= thermalManager.degTargetBed() + 2)
        {
          BLINK_LED(LED_BLINK_4);
        }
  ...
  else
  {
    if (card.isPrinting()) // lampeggio nella fase di preriscaldamento e stampa
    {
      if (thermalManager.degBed() < thermalManager.degTargetBed() - 2)
      {
        BLINK_LED(LED_BLINK_6); // lampeggio in fase riscaldamento piatto
      }
      if (thermalManager.degHotend(active_extruder) < thermalManager.degTargetHotend(active_extruder) - 2)
      {
...
#error "DISABLE_(DEBUG|JTAG) is not supported for the selected MCU/Board."
  ...
  serial_connect_timeout = millis() + 1000UL;
  while (/*!WIFISERIAL && */ PENDING(millis(), serial_connect_timeout))
  { /*nada*/
  }
#endif
